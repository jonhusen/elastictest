...
pull_es = f"{runtime} pull elasticsearch:{elastic_ver}"
pull_kb = f"{runtime} pull kibana:{elastic_ver}"
os.system(pull_es)
os.system(pull_kb)
...
es_name = "es-test-01"
mem_lim = "2GB"
start_es = f"{runtime} run --name {es_name} --net {elastic_net} -p 9200:9200 -it -m {mem_lim} elasticsearch:{elastic_ver}"
os.system(start_es)
...
kb_name = "kb-test-01"
start_kb = f"{runtime} run --name {kb_name} --net {elastic_net} -p 5601:5601 kibana:{elastic_ver}"
os.system(start_kb)

# Add elasticsearch node container
node_name = "es-test-02"
token = input("Elastic enrollment token:\n")
new_es_node = f"{runtime} run -e 'ENROLLMENT_TOKEN={token}' --name {node_name} --net {elastic_net} -m {mem_lim} elasticsearch:{elastic_ver} -d"
